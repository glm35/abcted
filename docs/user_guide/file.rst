...
abcde garde en mémoire les 10 fichiers les plus récemment utilisés et les
...
.. note:: la liste des fichiers récents est conservée dans le fichier texte
   codé en UTF-8 ``~/.config/abcde/recent_files.txt``.  Ce fichier peut être
   édité manuellement quand abcde est fermé.

Fichiers favoris
~~~~~~~~~~~~~~~~

abcde permet de marquer des fichiers récents comme favoris à l'aide du menu
...
.. note:: la liste des fichiers favoris est conservée dans le fichier texte
   codé en UTF-8 ``~/.config/abcde/favorite_files.txt``.  Ce fichier peut être
   édité manuellement quand abcde est fermé.
