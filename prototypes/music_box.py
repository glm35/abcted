...
This program also implements a failed attempt to change the tempo dynamically,
ie during the playback of the sequence.
...
def change_bpm(key):
    if ARGS.debug:
        print('<debug> changing bpm')
    bpm = SEQUENCER.beats_per_minute
    if key is '+':
        bpm += BPM_CHANGE_STEP
    elif key is '-':
        bpm -= BPM_CHANGE_STEP
    if bpm < 30 or bpm > 240:
        print('Error: bpm would be out of range, don\'t change it')
        print('Valid bpm range is [30-240]')
        return
...
def main():
    ...
    print('Playing...')
    print('Hit q, Q or Ctrl+C to stop')
    print("Hit '+' or '-' to change bpm")
    while True:
        key = getch()
